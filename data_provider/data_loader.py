...
class Dataset_Mirae_H(Dataset):
    ...
    def __read_data__(self):
        x = np.load('/nas/datahub/mirae/Data/x_H_total.npy')
        y = np.load('/nas/datahub/mirae/Data/y_H_total.npy')
        time = np.load('/nas/datahub/mirae/Data/time_H_index.npy')

        '''
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        num_train = int(len(x) * 0.7)
        num_val = int(len(x) * 0.85)
        border1s = [0, num_train, num_val]
        border2s = [num_train, num_val, len(x)]
