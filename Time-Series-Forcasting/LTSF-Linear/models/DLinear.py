    ...
    def forward(self, x, x_mark):
        ...
        if self.individual:
            ...
            if self.version == 'TWM-2':
                ...
                seasonal_init = seasonal_power * time + seasonal_init
                trend_init = trend_power * time + trend_init
                seasonal_output = self.Seasonal_Linear(seasonal_init.permute(0,2,1)).permute(0,2,1)
                trend_output = self.Trend_Linear(trend_init.permute(0,2,1)).permute(0,2,1)
            ...
            if self.version == 'D':
                ...
                seasonal_init = self.sigmoid(seasonal_init)
                trend_init = self.sigmoid(trend_init)
